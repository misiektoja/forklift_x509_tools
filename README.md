...
Simple shell scripts that allow for displaying and conversion of X.509 certificate / private key files in Forklift via Quick Open.

## List of scripts

- `ssl_display_cert.sh`: display details of DER/PEM/CRT/CER/P12/PFX/P7B certificate files
- `ssl_display_pkey.sh`: display details of RSA/EC DER/PEM/P12/PFX private key files
- `ssl_display_csr.sh`:  display details of CSR files
- `ssl_convert_cert.sh`: convert between different certificate formats and also allow splitting a PEM file containing multiple certificates into separate PEM files
- `ssl_display_crl.sh`:  display details of CRL files

## Requirements

The scripts require:
* Binarynights [Forklift 4.x](https://binarynights.com) file manager
* *openssl* binary
* GNU version of *sed*+*awk* which can be installed via [brew](https://brew.sh):
...
```
chmod a+x ssl_display_cert.sh ssl_display_csr.sh ssl_display_pkey.sh ssl_convert_cert.sh ssl_display_crl.sh
```
...
Set your own text editor in `ssl_display_crl.sh`.
...
For some scripts select **Show output** option (when something is being displayed), for others it is useful to have it disabled, but enable the option **Run for every selected file** (for example file conversion).
...
That's it. Now if you press the keyboard shortcut for Quick Open (Esc by default), you'll have the option to handle your X.509 files directly from the Forklift interface.
...
*/path_to_the_script/ssl_display_crl.sh "$SOURCE_SELECTION_PATHS"*
